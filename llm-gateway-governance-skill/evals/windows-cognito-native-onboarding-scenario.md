...
- [ ] Launchers run the **Python-3 preflight** (probe `py -3` then `python` with `--version`; accept only exit 0 + a `Python 3.x` banner) instead of a bare `if (Get-Command py) {...} else { & python ... }` fallback — on a box with no `py` and a Store-stub `python`, the launcher must print an actionable stderr error with `winget install Python.Python.3.12` and exit 1, never silently exit 9009; helper commands written into client configs use the absolute `sys.executable` + `%USERPROFILE%\.llm-gateway\gateway_auth.py` copy that `setup` installs.
