...
> ⚠️ **This is just one example of the general mechanism.** The core pattern is *"authorization unit (typically the org name) → LiteLLM team of the same name → per-team budget cap + model allowlist, set once via `TIER_CONFIG` or the Admin UI"*, and in practice you typically **create teams by org/team name and apply a budget to each org** (e.g. `org-research`, `team-frontend`). "economy/standard" is not a mandatory classification but just a label illustrating the pattern — replicate the flow below for as many orgs as you have. See `decision-tree.md` §3.
>
> This example uses **`org-sso`**, where the authorization unit is the **permission set name**. The identical mechanism applies to **`cognito-native`** — there the unit is the **Cognito User Pool Group name** (from the `cognito:groups` claim); everything else (same-named team, `TIER_CONFIG` first-creation seed, Admin UI thereafter) is unchanged.
...
| Layer | Role | File |
|---|---|---|
| **IAM Identity Center** | Assigns users to a tier via permission sets (`LlmGatewayUser` / `LlmGatewayEconomy`) — the permission set name is exactly the LiteLLM `team_alias` | (external, admin configuration) |
...
> **WHY unbranched (permission set = team_alias)?** Onboarding a *new* org later — say `LlmGatewayResearch` — needs zero
> Lambda changes: create the group + permission set named `LlmGatewayResearch` in IdC, then create/edit the
> `LlmGatewayResearch` team's budget/models in the LiteLLM Admin UI. `TIER_CONFIG` in this example only pre-seeds the
...
def _resolve_team_id(endpoint: str, master_key: str, team_alias: str) -> Optional[str]:
    """Unbranched mapping: team_alias is the permission set name."""
    seed = TIER_CONFIG.get(team_alias, {})
    return _ensure_team(
        endpoint, master_key, team_alias,
        models=seed.get("models"),
        max_budget=seed.get("max_budget"),
    )
...
- The model names in `TIER_CONFIG["LlmGatewayEconomy"]["models"]` must **exactly** match the names in LiteLLM `config.yaml`'s `model_list`.
...
- Logging in with the `LlmGatewayEconomy` permission set → the issued key belongs to the same-named `LlmGatewayEconomy` team, `models`=the 3 low-cost types, `max_budget`=50.
...
- The `LlmGatewayUser` permission set → the `LlmGatewayUser` team (unlimited, since `TIER_CONFIG` has no entry for it). Both tiers can use the AgentCore Web Search MCP.
