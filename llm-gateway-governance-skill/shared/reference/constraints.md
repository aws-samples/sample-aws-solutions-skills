...
- **⚠️ `privateDnsEnabled: true` on the `bedrock-agentcore` VPC endpoint breaks Web Search MCP (real-deploy incident).** Setting `privateDnsEnabled: true` creates a Route53 Private Hosted Zone for the ENTIRE `bedrock-agentcore.<region>.amazonaws.com` domain inside the VPC. That zone is authoritative — it holds only the endpoint's own A record — so the actual call target, the per-gateway subdomain `*.gateway.bedrock-agentcore.<region>.amazonaws.com`, NXDOMAINs instead of falling back to public DNS. Symptom: `mcp_servers.websearch` registers fine, `GET /v1/mcp/tools` or `tools/list` reports `"status":"unhealthy"`, and the LiteLLM container log shows `Error listing tools from websearch: [Errno -2] Name or service not known`. **Fix**: set `privateDnsEnabled: false` on this endpoint (`cdk-stacks.md` §1 NetworkStack) — LiteLLM never calls the control-plane API this endpoint fronts; it calls the Gateway's own public URL directly via SigV4. **Diagnosis tip**: verify with `aws route53 list-hosted-zones-by-vpc --vpc-id <vpc-id> --vpc-region <region>` — if a zone named `bedrock-agentcore.<region>.amazonaws.com.` is associated, that's the cause. **General principle**: any AWS service whose actual call target lives on a per-resource subdomain (gateway id, cluster id, etc.) below a VPC-endpoint-owned parent domain is at risk of this same NXDOMAIN trap — check before enabling `privateDnsEnabled: true` on a new endpoint type.
...
## ⛔ GATE: GPT-5.6 is deployable on the current pin, but MUST pass a post-deploy Codex smoke test (real-deploy incident history)

**GPT-5.6 (`gpt-5.6-sol` / `-terra` / `-luna`) is a valid model choice on the current image pin.** The two blockers that once made 5.6 undeployable are gone: Mantle accepts the Codex `namespace` tool type server-side (fixed 2026-07-19, verified on 5.4/5.5), and LiteLLM ships `gpt-5.6-sol/terra/luna` cost-map/routing entries in **stable** releases (PR #33412, first stable **v1.93.0**, 2026-07-18; this skill pins **v1.98.0**, on which the old `v1.89.0-rc.1` failure — a 5.6 route falling through to chat-completions emulation that Mantle rejects — no longer exists). **What remains gated: a deployment exposing any 5.6 alias MUST pass the smoke test below before Phase 6 hands the gateway to developers.** 5.6 has real-deploy incident history and open upstream issues; a green `cdk synth` is not evidence it works.

- **Variants and tiering** (all `bedrock_mantle/openai.gpt-5.6-*`; served in-region at the us-east-1 `bedrock-mantle` endpoint — the existing Mantle peering path, no network change): `gpt-5.6-sol` = flagship coding/agentic (1M context), `gpt-5.6-terra` = balanced default, `gpt-5.6-luna` = economy/latency tier. Keep `gpt-5.5` (proven flagship) / `gpt-5.4` (proven economy) offered alongside — they are the fallback pair if a 5.6 smoke test fails.
- **History — root-caused failure mode (do not re-diagnose from scratch if the symptom returns):** Codex CLI sets `namespace_tools: true` in its `ProviderCapabilities` for any provider using `wire_api = "responses"` (not `amazon-bedrock`-specific — the same wrapping fires for custom `model_providers.<id>` entries, which is how this gateway's Codex client config works). Once any real config with tools is in play, Codex serializes tool definitions in an OpenAI-proprietary `{"type": "namespace", ...}` wrapper; Mantle's Responses validator originally accepted only `"function"`/`"mcp"` and rejected it with `400 validation_error: ... unknown variant 'namespace'`. Codex surfaces the hard 400 as a misleading **"Reconnecting... N/5 — We're currently experiencing high demand"** loop. Reproduced end-to-end on `gpt-5.6-sol` (CloudWatch: 138/149 `/v1/responses` calls in a live session returned 500; only single-turn/no-tool requests got 200). AWS then fixed `namespace` server-side for 5.4/5.5 (`openai/codex#25034` closed with no Codex-side change; direct `/v1/responses` namespace-wrapped test returned 200 on both).
- **Open upstream issues to re-check at deploy time** (BerriAI/litellm, status as of 2026-08-24):
  - **#36545 / #36546** — Codex CLI **≥ 0.147** hoists tools into a namespace literally named `functions`, colliding with a reserved Mantle namespace (`400: "User-defined namespace 'functions' collides with an existing tool namespace"`), again masked as "high demand". This is per **Codex CLI version**, not per model — record the Codex version used in the smoke test.
  - **#33075** — Claude Code's background auto-classifier via `/v1/messages` can fail on GPT-5.6 (stop_sequences translation). Relevant only if a 5.6 alias is configured as a Claude Code model.
  - **#31475 / #38028** — native SigV4 / per-model static-key auth bugs on the Mantle route. This skill's Bearer-callback auth is unaffected (see "LiteLLM image + Mantle Bearer-token auth" below).
- **⚠️ Codex routed to `amazon-bedrock` instead of the gateway — a FOURTH cause of the same misleading symptom, and a governance-bypass risk. ROOT-CAUSED to two bugs in `_merge_codex_config` (this skill's onboarding merger), NOT to Codex behavior (2026-08-25):** on a developer machine that already had `model_provider = "amazon-bedrock"` in `~/.codex/config.toml` (left over from AWS's own "Configure Codex with Amazon Bedrock" workshop / a prior direct-Bedrock setup), `codex exec` printed `provider: amazon-bedrock` in its session banner and sent requests straight to `https://bedrock-mantle.<AWS_REGION>.api.aws/openai/v1/responses` — using the gateway region (us-east-2) instead of Mantle's us-east-1 — so it surfaced as `404 ... The model 'gpt-5.6-sol' does not exist`, rendered as the misleading `Reconnecting... 1/5…5/5` loop. **Codex was behaving correctly** — it honored the `model_provider` its config named. The bug was ours, in two places: **(1) preserve-and-warn merge policy** — the merger treated `model_provider`/`model`/`web_search` as ordinary user data ("keep the existing value, just log a note"), so it never replaced the stale `amazon-bedrock` value; **(2) block-removal regex `[^\[]*`** — that pattern stops at the next literal `[`, but our own `[model_providers.llm-gateway.auth]` block contains `args = []` (a literal `[`), so the old block was never actually removed and each run appended a duplicate, eventually producing a `duplicate key` TOML parse error. **Fix (shipped, see `developer-onboarding.md` §1A):** `model_provider`/`model`/`web_search` are now **force-set** (`force_upsert`, authoritative overwrite — they are the switch that decides whether Codex talks to the gateway at all, not user preference), and the block is removed by a **line-based** regex (`^\[model_providers\.llm-gateway(?:\.auth)?\]\n(?:(?!\[)[^\n]*\n?)*`) that tolerates `[`/`]` inside a value. The one-off CLI workaround (`codex exec -c model_provider=llm-gateway -c model=gpt-5.6-sol`) is **no longer needed** and must not be shipped as a wrapper — `setup` writes an authoritative `config.toml`. **Why this matters beyond a broken test:** if the config silently names `amazon-bedrock`, the session only "works" when the developer's own AWS creds happen to have direct Mantle access — and if they do, every prompt/response bypasses LiteLLM's virtual keys, Guardrails, budget caps, and usage tracking with no error surfaced. The Phase-5 smoke test must therefore check *which provider actually handled the call* (the banner's `provider:` line), not just "did it succeed".
- **Mandatory 5.6 smoke test (Phase 5, before Phase 6 onboarding):** run a fresh **multi-turn, tool-using Codex session** against each exposed 5.6 alias — top-level `web_search = "disabled"` (a `web_search` tool produces the *same* misleading "high demand" symptom and must not contaminate the test), default MCP servers registered. **First confirm the session banner's `provider:` line matches the gateway's configured provider name** (`llm-gateway`, not `amazon-bedrock`). If it shows `amazon-bedrock`, `setup` did not write an authoritative `config.toml` — re-run `gateway_auth.py setup` and check that `~/.codex/config.toml` has `model_provider = "llm-gateway"` at top level with no duplicate `[model_providers.llm-gateway]` blocks (the force-set + line-based-regex fix; see the routing entry above). Do not paper over it with per-invocation `-c` overrides — a correctly merged config needs none. Then verify in CloudWatch Logs that the session's `/v1/responses` calls return 200; any `namespace`/`unknown variant` `validation_error` is the incident pattern above. If a 5.6 alias is also exposed to Claude Code, run one Claude Code session against it too (#33075).
- **If the smoke test fails:** de-alias/remove the failing 5.6 entry and ship on `gpt-5.5`/`gpt-5.4`; record the Codex CLI version + the CloudWatch error signature (it is most likely a Codex-version issue like #36545, not a config bug). Never hand a failing 5.6 alias to developers.
- **Verify the `gpt-5.5`/`gpt-5.4` fallback pair actually exists in this account/region BEFORE promising it as a fallback.** `aws bedrock list-inference-profiles` in a real deploy showed `gpt-5.6-sol/terra/luna` and `gpt-oss-*` profiles but NO `gpt-5.5`/`gpt-5.4` profile at all — the fallback pair this skill's Discovery/Design phase assumes is not universally available. **Action**: at Design time, run `aws bedrock list-inference-profiles` (or `list-foundation-models --by-provider openai`) and grep for the exact fallback model names before adding them to `constants.ts`'s `model_list`. If absent, do NOT wire them into `services/litellm/config.yaml`'s active `model_list` (defining them in `constants.ts` for forward-compatibility is fine; routing to a nonexistent profile ID fails every call). Tell the user explicitly that no fallback exists for this deployment if the smoke test fails and there's nothing to fall back to.
- **Action for the skill**: `lib/config/constants.ts` `MODELS` defines `GPT56_SOL`/`GPT56_TERRA`/`GPT56_LUNA` alongside `GPT55`/`GPT54`; `config.yaml` routes the tiers selected at Discovery. ⚠️ On an **already-deployed** environment, adding models does NOT update existing teams' allowlists — `TIER_CONFIG` seeds only a team's first-ever creation (Hard Constraint #13); update each existing team's `models` via the LiteLLM Admin UI or `POST /team/update` (see `litellm-admin-guide.md`).
...
> ⚠️ **Status of Mantle SigV4, per pinned tag (always verify against the actual installed source, never release notes).** On the retired `v1.89.0-rc.1` pin the `bedrock_mantle` Responses route had **no SigV4 code path at all** — `validate_environment()` read `BEDROCK_MANTLE_API_KEY` / `AWS_BEARER_TOKEN_BEDROCK` and raised `ValueError` if neither was set (the even earlier "SigV4 shipped in v1.87.2 / #29788" claim was false and stays retracted). On the **current `v1.98.0` pin** (source extracted from the image 2026-08-24, `litellm/llms/bedrock_mantle/common_utils.py` — note the package now lives under `/app/.venv/lib/python3.13/site-packages/`): `BedrockMantleAuthMixin.sign_request` uses a Bearer when present (`litellm_params.api_key` → `BEDROCK_MANTLE_API_KEY` → `AWS_BEARER_TOKEN_BEDROCK`) and **only otherwise** falls back to SigV4 signed with service name `"bedrock"` — a fallback whose signing service name is disputed upstream (BerriAI/litellm#31475, open; sibling: #38028, per-model static AWS keys ignored). **This skill does not use the fallback**: the Bearer callback below keeps `BEDROCK_MANTLE_API_KEY` fresh, and a present Bearer takes precedence in the source, so auth stays deterministic. If the pinned tag changes again, re-extract and re-read `common_utils.py` + `responses/transformation.py` before believing any auth claim.
...
- **`get_secret_str()`/`os.getenv()` are re-read per request (no caching for plain env vars in this build)**, confirmed in `litellm/secret_managers/main.py` (re-verified on the v1.98.0 image: no cache decorator on `get_secret*`) — so the callback updating `os.environ` takes effect immediately, no LiteLLM restart.
- **Dockerfile: the base image has no `pip`** (uv-managed venv, pip stripped — re-verified on v1.98.0: `python3 -m pip` → `No module named pip`, and `aws-bedrock-token-generator` is still not bundled). To add it, copy the `uv` binary from `ghcr.io/astral-sh/uv:latest` in a separate stage and run `uv pip install --python /app/.venv/bin/python3 aws-bedrock-token-generator==1.1.0`.
...
- **⚠️ `cdk deploy <subset> --outputs-file outputs.json` OVERWRITES outputs.json with ONLY that subset's outputs (real-deploy incident).** `cdk deploy LiteLLMStack --outputs-file outputs.json` after a full `--all` deploy replaces the file's content with just `LiteLLMStack`'s outputs — every other stack's outputs (AuthStack's `TokenServiceUrl`/`CognitoHostedUiDomain`/`CognitoAppClientId`, etc.) are silently deleted from the file. Any downstream tool that reads the merged `outputs.json` (e.g. `gateway_auth.py setup`) then writes a config with empty strings for the missing fields — in one real incident this produced `authorizationEndpoint: "https:///oauth2/authorize"` (no host), which manifested only as "the login URL looks wrong" far downstream, not as an obvious error at deploy time. **Fix / practice**:
  - For any partial/single-stack deploy after the initial full deploy, write outputs to a **separate temp file** (`--outputs-file /tmp/<stack>-outputs.json`), never directly overwrite the canonical `outputs.json`.
  - To refresh the canonical `outputs.json` with all stacks' current outputs without changing anything, run `cdk deploy --all --exclusively --outputs-file outputs.json` — each stack no-ops if unchanged but still reports its outputs, so the file is fully repopulated.
  - Any onboarding script that consumes `outputs.json` (`gateway_auth.py setup`) should validate that required fields are non-empty and **fail loudly** rather than silently writing a broken config — see the companion fix in `developer-onboarding.md` §1A.
...
1. **⚠️ Do NOT use `--forwarded-allow-ips` (real-deploy incident).** Earlier revisions of this skill told the entrypoint to run `litellm ... --forwarded-allow-ips '*'` so uvicorn would trust `X-Forwarded-Proto`/`X-Forwarded-Host`. **The pinned image's `litellm` CLI does not have that option** — the container exits instantly (`Error: No such option: --forwarded-allow-ips`, exitCode 2), the ECS circuit breaker retries 5x and rolls the deploy back. Verified against the actual image (and re-verified on the current v1.98.0 pin — `litellm --help` still has no such flag): `proxy_cli.py` constructs the uvicorn args explicitly and reads neither the flag nor a `FORWARDED_ALLOW_IPS` env var, so there is no uvicorn-level workaround. Redirect correctness comes from `PROXY_BASE_URL` instead.
...
- **Three traps that come with key expiry** (all handled in `lambda-handlers.md` — do not regress; each caused or nearly caused a real outage):
  ...
  - *Recovery itself crashing (real ~30 min outage)*: the `user_id` can contain `:` and `+` (`<acct>:user+tag` in org-sso, or a plus-addressed email `user+tag@example.com` in cognito-native) and **must be percent-encoded** in the `user/info` query — a raw `+` arrives as a space, LiteLLM 404s "user not found", and an unhandled HTTPError turns every issuance into a 500 until LiteLLM's periodic expired-key cleanup happens to free the alias. Also: **`/key/generate` does not create a user record**, so `user/info` can legitimately 404 even when the alias row exists — recovery must fall back to deleting the alias and re-creating (in practice this delete+recreate path IS the normal rotation on such deployments).
...
- **id_token vs access_token (log-identity trap)**: a Cognito **access token carries no `email` claim** — only the id_token does. If the client sends the access token, the Token Lambda has no email and LiteLLM logs an opaque `sub` UUID as `user_id`. Fix: set **no `authorizationScopes`** on the `COGNITO_USER_POOLS` authorizer (scopes exist only on access tokens and would force access-token-only), and have the client helper send the **id_token** — it carries both `email` and `cognito:groups`, so team mapping is unaffected. (Real incident: logs showed `cognito-native:915b9530-…` until this was flipped.)
...
- **`gateway_auth.py setup`'s installed copy at `~/.llm-gateway/gateway_auth.py` MUST be `chmod 700` on POSIX, not just `chmod 600`.** Codex's `model_providers.<id>.auth.command` execs this file directly (no shell), so a copy without the executable bit makes every Codex API call fail immediately with `Permission denied (os error 13)` — logged only in Codex's own `RUST_LOG=debug` output, invisible from the LiteLLM/gateway side, so it is easy to mistake for a network or auth-token problem. `shutil.copyfile()` does not preserve the source's execute bit; set it explicitly after copying (golden code: `developer-onboarding.md` §1A `cmd_setup`).
