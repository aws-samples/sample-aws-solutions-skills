...
The launchers resolve their own real path (bash: a `readlink` loop over `$BASH_SOURCE`; PowerShell: `$PSScriptRoot`) so they work from **any** working directory — including a `~/.local/bin` symlink or a PowerShell-profile function. Do not write launchers that assume the repository cwd. **Every `.ps1` launcher must end with `exit $LASTEXITCODE`** (PowerShell 5.1 does not propagate native exit codes otherwise — the token helper's non-zero-exit contract breaks silently) and **must run the §1A Python-3 preflight** — probe `py -3` then `python` with `--version`, accept only a real `Python 3.x` banner + exit 0, and fail on **stderr** with a `winget install Python.Python.3.12` hint when neither qualifies. Preferring `py -3` alone is NOT enough: on a clean box `py` can be absent AND `python` the Microsoft Store alias stub (no output, exit 9009), and a bare if/else fallback then "does nothing" (real onboarding incident — see `developer-onboarding.md` §1A).
