...
| **Amazon Bedrock (Mantle)** | OpenAI-family (GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.4) via the Responses route (`bedrock_mantle/`) | **Bearer-token auth** — a short-term Bedrock API key is minted at runtime from the Task Role via `aws-bedrock-token-generator` into env **`BEDROCK_MANTLE_API_KEY`** (never `AWS_BEARER_TOKEN_BEDROCK` — boto3-reserved, breaks Claude); the v1.98.0 Bearer-less SigV4 fallback is upstream-disputed (#31475) and bypassed by a present Bearer. **Not** Guardrail-compatible. Pinned to **us-east-1** (all offered tiers in-region there) → reached over cross-region VPC peering. Pin via `aws_region_name=us-east-1` + env `BEDROCK_MANTLE_REGION` + `BEDROCK_MANTLE_API_BASE` (NOT `MANTLE_REGION`). AWS Marketplace offering (auto-subscribe). IAM on `project/*`, not `foundation-model`. |
...
| **Amazon API Gateway (REST)** | Token Service edge | `org-sso`: `AuthorizationType.IAM` (SigV4). `cognito-native`: `AuthorizationType.COGNITO` (`CognitoUserPoolsAuthorizer`, **no `authorizationScopes`** so it accepts the ID token — the client sends the id_token for its `email` claim). |
...
| GPT (5.x family: `gpt-5.6-sol`/`-terra`/`-luna`, `gpt-5.5`, `gpt-5.4`) | `bedrock_mantle/openai.<model-id>` | **Bearer token** (short-term Bedrock API key minted at runtime from the Task Role → env `BEDROCK_MANTLE_API_KEY`; never `AWS_BEARER_TOKEN_BEDROCK`; a present Bearer bypasses v1.98.0's upstream-disputed SigV4 fallback — see constraints.md); `aws_region_name`+`BEDROCK_MANTLE_REGION`+`BEDROCK_MANTLE_API_BASE`=us-east-1 | ❌ not compatible |

Routing lives in `lib/config/constants.ts` (`MODELS`) and `services/litellm/config.yaml` (env-driven). Aliases are what tools like Claude Code / Codex send; keep them stable. GPT tiering: `gpt-5.6-sol` = flagship coding/agentic (1M context), `gpt-5.6-terra` = balanced, `gpt-5.6-luna` = economy/latency, with `gpt-5.5` (proven flagship) / `gpt-5.4` (proven economy, ≈2x cheaper than 5.5) as the fallback pair. ⛔ Any `gpt-5.6-*` alias requires the post-deploy Codex smoke test before onboarding (constraints.md GPT-5.6 gate).
