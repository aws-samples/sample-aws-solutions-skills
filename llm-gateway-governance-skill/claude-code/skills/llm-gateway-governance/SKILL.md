...
2. **Models?** Which Claude / GPT(Mantle) models? **Per-org governance (optional)**: which SSO groups/teams need their own budget cap + model allowlist? Each SSO permission set maps 1:1 to a same-named LiteLLM team (no code branching per org) — so all Discovery needs to capture is: what should each group's **permission-set name** be, and what initial `models`/`max_budget` (if any) should that team start with. Onboarding additional orgs later is then IdC + LiteLLM Admin UI only, never a code change. ("economy/standard" is just one illustrative naming — not a required split; name permission sets/teams after real orgs/teams.)
...
- **Token Lambda tiering (Hard Constraints #13)**: generate `_resolve_team_id` so the SSO permission set maps 1:1, unbranched, to a same-named LiteLLM team — never emit an `if permission_set in {...}` per-org branch. Seed `TIER_CONFIG` only with the org(s) named in Discovery (as a starter `models`/`max_budget` for that team's first-ever creation); this is what keeps future onboarding console-only (IdC + LiteLLM Admin UI), not a Lambda redeploy. See `shared/patterns/lambda-handlers.md`.
...
13. **SSO permission set = LiteLLM team_alias, unbranched.** `_resolve_team_id` must map the caller's permission set 1:1 to a same-named team — never hard-code an `if permission_set in {...}` org/tier list in `handler.py`. Onboarding a new org after deploy must be achievable purely via IdC (group + permission set) + LiteLLM Admin UI (Teams → set models/budget), with **no Lambda code change or redeploy**. `TIER_CONFIG` may seed a team's *first-ever* creation with initial `models`/`max_budget` from Discovery answers, but is never consulted again once a human has touched that team in the Admin UI. See `shared/patterns/lambda-handlers.md`.
